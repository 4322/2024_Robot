...
public class SwerveModuleIOMotorControl implements SwerveModuleIO {
  ...
  private double[] feedForwardRPSThreshold =
      DriveConstants.Drive.FeedForward.feedForwardRPSThreshold.clone();
  private double[] feedForwardVolts =
      DriveConstants.Drive.FeedForward.voltsAtSpeedThresholds.clone();
  ...
  private void configRotation(TalonFX talonFX) {
    ...
    // encoder.setPositionConversionFactor(360);  TODO: rewrite code to operate on 0-1 rotations
    // instead of 0-360 degrees. convert encoder position duty cycle to degrees
  ...
  public void setDriveVoltage(double desiredMotorRPM) {
    ...
    if (i == feedForwardRPSThreshold.length - 1) {
      ...
      double slope =
          (feedForwardVolts[lastElement] - feedForwardVolts[secondToLastElement])
              / (feedForwardRPSThreshold[lastElement]
                  - feedForwardRPSThreshold[secondToLastElement]);

      calculatedFeedForwardValue =
          slope * (desiredMotorRPS - feedForwardRPSThreshold[lastElement])
    ...
    // Calculated through weighted average.
      ...
      double weight =
          (desiredMotorRPS - feedForwardRPSThreshold[upperBound])
              / (feedForwardRPSThreshold[lowerBound] - feedForwardRPSThreshold[upperBound]);

      calculatedFeedForwardValue =
          (weight * feedForwardVolts[lowerBound]) + ((1 - weight) * feedForwardVolts[upperBound]);
    }
    ...
    double desiredVolts =
        (kSVolts * Math.signum(desiredMotorRPM)) + (calculatedFeedForwardValue * desiredMotorRPS);

    // send requested voltage to SparkMAX

    driveMotor.setVoltage(desiredVolts);
    /**
     * if (error != REVLibError.kOk) { DriverStation.reportError("Drive motor " +
     * driveMotor.getDeviceId() + " error " + error.name() + " while sending requested voltage",
     * false); }*
     */
  }
